...
def setup_australia(min_votes=20):
    ...
    df = pd.read_stata('%s/original_data_files/HCJD_Australia.dta'%DATADR, convert_categoricals=False)

    # Just get the votes. These columns end in v1 and v2 (votes on issues 1 and 2).
    colsToKeep = np.sort([c for c in df.columns if 'v_' in c])
    df = df[colsToKeep]
    

    # Run some checks.
    totalVotes = (np.isnan(df)==0).sum(1)>7
    if (totalVotes>7).any():
        print("There are votes with more than 7 voters which should be impossible.\n")
    voteTypes = np.unique(df)
    voteTypes = voteTypes[np.isnan(voteTypes)==0].astype(int)
    if not set(voteTypes)<=set(range(4)):
        print("There are vote types that are not in the specified codebook range of 0-3.")
        print(voteTypes)
        print()

    # Find all natural courts that had full votes by looking for groups of seven voters. 
    # We can do this by getting the list of sorted justice indices and taking the unique
    # set of 7 voters.
    fullVotesIx = np.where( (np.isnan(df)==0).sum(1)==7 )[0]
    assert fullVotesIx.size>0, "There are no full votes!"
    ...
    # Only taking data sets with more than min votes...
    courts = []
    for f in fullNatCourtVotes:
        if f.shape[0]>min_votes:
            courts.append({})
            courts[-1]['justices'] = [c.split('_')[1] for c in f.columns]
            courts[-1]['votes'] = f.values
    
    print("Overwriting Australian court information...")
    pickle.dump({'courts':courts}, open('%s/australian_full_court_votes.p'%DATADR,'wb'), -1)

def setup_india(keepv2=True, min_votes=20):
    """2019-02-19"""
    df = pd.read_stata('%s/original_data_files/HCJD_India.dta'%DATADR, convert_categoricals=False)
    ...
    print("Overwriting Indian court information...")
    pickle.dump({'courts':courts}, open('%s/india_full_court_votes.p'%DATADR,'wb'), -1)
